...
5. No hand-kept index pages. At 1st the tag is the index.
6. A 2nd tag index is generated by `wiki-update`. Never hand-kept.
...
## 2nd shapes
1. 2nd holds **two species**. The name tells them apart.
2. Tag index — `{YYMMDD}_{HHMM}_{tag}.md`. The timestamp prefix marks it.
3. `wiki-update` writes it. Links only — no prose, no images.
4. Title, then `#2nd #{tag}`, then one `>` line, then numbered hub links.
5. Hubs listed oldest first.
6. Built only when 2 or more hubs carry the tag.
7. The timestamp is the trigger page's `createdTime`, not the tag's.
8. Synthesis — bare name, no timestamp. Prose, images, hand-written.
9. The skill never writes or edits a synthesis page.
10. Compression does not apply to an index. There is no distilled body.
