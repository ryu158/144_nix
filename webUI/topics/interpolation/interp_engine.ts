 ...
 * Additive utility — grid.ts / chart.ts / dual-chart.ts remain untouched.
 */
interface Point { x: number; y: number; }

const InterpEngine = {
  // Sorted {x,y} points for one series column (1-based) from a grid2d table.
  _seriesPoints(data2d: Grid2D, colIndex: number): Point[] {
    const pts: Point[] = [];
  ...
  _interpAt(points: Point[], xQuery: number): number | null {
  ...
  _round(v: number, digits = 6): number {
    const f = Math.pow(10, digits);
    return Math.round(v * f) / f;
  },

  // Public: Input data2d + query X's -> Output data2d (one Y col per Input Y series).
  buildOutputTable(inputData2d: Grid2D, queryXs: number[]): Grid2D {
	...
	generateRange(xMinRaw: string | number, xMaxRaw: string | number, xIntervalRaw: string | number): number[] {
