...
const InterpEngine = {
    ...
    _interpAt(points, xQuery) {
        if (points.length === 0)
            return null;
        if (xQuery < points[0].x || xQuery > points[points.length - 1].x)
            return null;
        let lo = 0, hi = points.length - 1;
        while (hi - lo > 1) {
            const mid = (lo + hi) >> 1;
            if (points[mid].x <= xQuery)
                lo = mid;
            else
                hi = mid;
        }
    ...
    buildOutputTable(inputData2d, queryXs) {
        ...
        for (let s = 1; s <= seriesCount; s++)
            seriesPoints.push(this._seriesPoints(inputData2d, s));
    ...
    generateRange(xMinRaw, xMaxRaw, xIntervalRaw) {
        ...
        if ([xMin, xMax, xInterval].some(Number.isNaN))
            return [];
        if (xInterval <= 0 || xMax < xMin)
            return [];
        const steps = Math.floor((xMax - xMin) / xInterval + 1e-9);
        const xs = [];
        for (let i = 0; i <= steps; i++)
            xs.push(this._round(xMin + i * xInterval, 9));
        return xs;
    },
};
//# sourceMappingURL=interp_engine.js.map