...
class GridTable {
    ...
    setData(arr2d) {
        ...
        this.colWidths = new Array(this.numCols).fill(this.defaultColWidth);
        this.rowHeights = new Array(this.numRows).fill(this.defaultRowHeight);
        this.data = Array.from({ length: this.numRows }, (_, r) => Array.from({ length: this.numCols }, (_, c) => arr2d[r]?.[c] ?? ''));
        this._computeOffsets();
    ...
    on(event, cb) {
        var _a;
        ((_a = this._listeners)[event] || (_a[event] = [])).push(cb);
    }
    destroy() {
    ...
    _indexForOffset(offsets, pos) {
        ...
        if (pos <= 0)
            return 0;
        if (pos >= offsets[offsets.length - 1])
            return offsets.length - 2;
        while (lo <= hi) {
            const mid = (lo + hi) >> 1;
            if (offsets[mid] <= pos && pos < offsets[mid + 1])
                return mid;
            if (offsets[mid] > pos)
                hi = mid - 1;
            else
    ...
    _ensureSize(rows, cols) {
        ...
        if (cols > this.numCols) {
            for (let r = 0; r < this.numRows; r++) {
                while (this.data[r].length < cols)
                    this.data[r].push('');
            }
            for (let c = this.numCols; c < cols; c++)
    ...
    _buildDom() {
        ...
        if (this.viewportWidth)
            this.root.style.width = this._toCssSize(this.viewportWidth) ?? '';
    ...
    renderViewport() {
        ...
        c0 = Math.max(0, c0);
        c1 = Math.min(this.numCols, c1);
        r0 = Math.max(0, r0);
        r1 = Math.min(this.numRows, r1);
        // Fast DOM Clears
        ...
        if (this.editingCell) {
            const el = this.cellsEl.querySelector(`.gt-cell[data-row="${this.editingCell.r}"][data-col="${this.editingCell.c}"]`);
            if (el)
                this._activateEditableEl(el);
        }
    }
    _requestRender() {
    ...
    _startEdit(r, c, initialChar = null) {
        if (this.readOnly)
            return; // 👇 Block editing early
        this._setSelection(r, c, r, c);
        this.editingCell = { r, c, original: this.data[r][c] ?? '' };
        if (initialChar !== null)
            this.data[r][c] = initialChar;
        this._requestRender();
    }
    ...
    _onHiddenKeydown(e) {
        ...
        switch (e.key) {
            ...
            case 'Backspace': {
                e.preventDefault();
                if (this.readOnly)
                    return; // 👇 Block single/multi-cell deletion
                const sel = this._normSelection();
            ...
            case 'ArrowUp':
                e.preventDefault();
                this._moveActive(-1, 0, e.shiftKey);
                break;
            case 'ArrowDown':
                e.preventDefault();
                this._moveActive(1, 0, e.shiftKey);
            ...
            case 'ArrowLeft':
                e.preventDefault();
                this._moveActive(0, -1, e.shiftKey);
                break;
            case 'ArrowRight':
                e.preventDefault();
                this._moveActive(0, 1, e.shiftKey);
    ...
    _moveActive(dr, dc, extend = false) {
        if (extend) {
            ...
            this.selection.r2 = r2;
            this.selection.c2 = c2;
        }
        else {
    ...
    _scrollCellIntoView(r, c) {
        ...
        else if (right > sl + viewW)
            this.scrollEl.scrollLeft = right - viewW;
        if (top < st)
            this.scrollEl.scrollTop = top;
        else if (bottom > st + viewH)
            this.scrollEl.scrollTop = bottom - viewH;
    }
    _onCopy(e, isCut = false) {
        ...
        for (let r = sel.r1; r <= sel.r2; r++) {
            const rowVals = [];
            for (let c = sel.c1; c <= sel.c2; c++)
                rowVals.push(this.data[r][c] ?? '');
            rows.push(rowVals.join('\t'));
        }
        e.clipboardData?.setData('text/plain', rows.join('\n'));
        if (isCut) {
            if (this.readOnly)
                return; // 👇 Block cutting (copying will still function)
            for (let r = sel.r1; r <= sel.r2; r++) {
                for (let c = sel.c1; c <= sel.c2; c++)
                    this.data[r][c] = '';
            }
    ...
    _onPaste(e) {
        e.preventDefault();
        if (this.readOnly)
            return; // 👇 Block clipboard pasting
    ...
    _showColMenu(e, c) {
        ...
        if (!this.fixedColCount) {
            menuItems.push({ label: `Insert ${noun} left`, action: () => this._insertCols(c1, count) }, { label: `Insert ${noun} right`, action: () => this._insertCols(c2 + 1, count) }, { sep: true });
        }
    ...
    _deleteRows(at, count = 1) {
        if (this.readOnly)
            return; // 👇 Block row structural deletes
