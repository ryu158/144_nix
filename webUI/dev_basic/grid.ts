...
interface GridOptions {
  ...
  viewportWidth?: CssSize | null;
  readOnly?: boolean;
}

interface GridSelection { r1: number; c1: number; r2: number; c2: number; }
interface GridActiveCell { r: number; c: number; }
interface GridEditingCell { r: number; c: number; original: string; }
interface GridAxisRef { type: 'col' | 'row'; index: number; }
interface GridGlobalListener { target: EventTarget; type: string; handler: EventListener; }
interface GridMenuItem { label?: string; sep?: boolean; danger?: boolean; action?: () => void; }

type GridEventHandler = (payload?: unknown) => void;

class GridTable implements GridSource {
  ...
  // Set by _computeOffsets(), which the constructor calls before first use.
  colOffsets!: number[];
  rowOffsets!: number[];
  totalWidth!: number;
  totalHeight!: number;
  ...
  colHeaderEl!: HTMLDivElement;
  rowHeaderEl!: HTMLDivElement;
  cellsEl!: HTMLDivElement;
  hiddenInput!: HTMLTextAreaElement;

  protected _toCssSize: (v: CssSize | null | undefined) => string | null;
  protected _dragging: boolean;
  protected _resizing: GridAxisRef | null;
  protected _listeners: Record<string, GridEventHandler[]>;
  protected _renderPending: boolean;
  protected _globalListeners: GridGlobalListener[];
  protected _headerDragging: 'col' | 'row' | null = null;
  protected _headerAnchor: GridAxisRef | null = null;
  protected _menuEl: HTMLDivElement | null = null;
  ...
  constructor(container: HTMLElement, options: GridOptions = {}) {
    ...
    this._toCssSize = (v: CssSize | null | undefined) => (v === null || v === undefined) ? null : (typeof v === 'number' ? v + 'px' : v);
    this.buffer = 3;
  ...
  on(event: string, cb: GridEventHandler) {
  ...
  protected _emit(event: string, payload?: unknown) {
    (this._listeners[event] || []).forEach(cb => cb(payload));
  }

  protected _computeOffsets() {
    this.colOffsets = [0];
  ...
  protected _indexForOffset(offsets: number[], pos: number): number {
  ...
  protected _ensureSize(rows: number, cols: number) {
    let changed = false;
	...
	static colLabel(n: number): string {
		let v = n + 1;
  ...
  renderViewport() {
    ...
    if (this.editingCell) {
      const el = this.cellsEl.querySelector<HTMLElement>(
  ...
  protected _buildColHeaderCell(c: number, sl: number): HTMLDivElement {
  ...
  protected _buildRowHeaderCell(r: number, st: number): HTMLDivElement {
    const el = document.createElement('div');
  ...
  protected _buildDataCell(r: number, c: number, sl: number, st: number, sel: GridSelection): HTMLDivElement {
    const el = document.createElement('div');
  ...
  protected _onCellMouseEnter(r: number, c: number) {
  ...
  protected _onColHeaderMouseDown(e: MouseEvent, c: number) {
  ...
  protected _onColHeaderMouseEnter(c: number) {
    if (this._headerDragging !== 'col') return;
    this.selection.c2 = c;
    this._requestRender();
  }

  protected _onRowHeaderMouseDown(e: MouseEvent, r: number) {
    if (this.editingCell) this._commitEdit();
    e.preventDefault();
  ...
  protected _onRowHeaderMouseEnter(r: number) {
    if (this._headerDragging !== 'row') return;
    this.selection.r2 = r;
    this._requestRender();
  }

  protected _onCornerMouseDown(e: MouseEvent) {
    e.preventDefault();
    if (this.editingCell) this._commitEdit();
    this._selectAll();
    this.hiddenInput.focus({ preventScroll: true });
  ...
  protected _startEdit(r: number, c: number, initialChar: string | null = null) {
  ...
  protected _activateEditableEl(el: HTMLElement) {
    el.classList.add('editing');
    el.contentEditable = 'true';
    el.spellcheck = false;
    
    const keydownHandler = (e: KeyboardEvent) => this._onEditKeydown(e);
  ...
  protected _onEditKeydown(e: KeyboardEvent) {
    if (e.key === 'Enter') {
      e.preventDefault();
      this._commitEdit();
  ...
  protected _addGlobalListener(target: EventTarget, type: string, handler: EventListener) {
  ...
  protected _bindEvents() {
    ...
    this._addGlobalListener(document, 'keydown', (e: Event) => {
      if ((e as KeyboardEvent).key === 'Escape') this._closeMenu();
    });
  ...
  protected _onHiddenKeydown(e: KeyboardEvent) {
  ...
  protected _moveActive(dr: number, dc: number, extend = false) {
    if (extend) {
  ...
  protected _scrollCellIntoView(r: number, c: number) {
  ...
  protected _onCopy(e: ClipboardEvent, isCut = false) {
    e.preventDefault();
    const sel = this._normSelection();
    const rows: string[] = [];
    for (let r = sel.r1; r <= sel.r2; r++) {
	...
	protected _onPaste(e: ClipboardEvent) {
		e.preventDefault();
  ...
  protected _startColResize(e: MouseEvent, c: number) {
    e.preventDefault();
    e.stopPropagation();
  ...
  protected _startRowResize(e: MouseEvent, r: number) {
    e.preventDefault();
    e.stopPropagation();
  ...
  protected _openMenu(x: number, y: number, items: GridMenuItem[]) {
    this._closeMenu();
    ...
    const onOutside = (ev: MouseEvent) => {
      if (!menu.contains(ev.target as Node)) { this._closeMenu(); document.removeEventListener('mousedown', onOutside); }
    };
    setTimeout(() => document.addEventListener('mousedown', onOutside), 0);
  }

  protected _showRowMenu(e: MouseEvent, r: number) {
    e.preventDefault();
    const sel = this._normSelection();
  ...
  protected _showColMenu(e: MouseEvent, c: number) {
    e.preventDefault();
    const sel = this._normSelection();
...
if (!this.fixedColCount) {
  ...
  protected _clampSelection() {
    const clampR = (v: number) => Math.min(this.numRows - 1, Math.max(0, v));
    const clampC = (v: number) => Math.min(this.numCols - 1, Math.max(0, v));
    this.selection = {
  ...
  protected _insertRows(at: number, count = 1) {
  ...
  protected _deleteRows(at: number, count = 1) {
	...
	protected _insertCols(at: number, count = 1) {
		if (this.fixedColCount) {
  ...
  protected _deleteCols(at: number, count = 1) {
    if (this.readOnly) return; // 👇 Block row structural deletes
