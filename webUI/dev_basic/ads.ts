...
loadScript() {
  ...
  renderUnit(container: HTMLElement) {
  ...
  init(containerId: string) {
	...
	renderFixedUnit(container: HTMLElement, width: number, height: number) {
		const ins = document.createElement('ins');
		ins.className = 'adsbygoogle';
	...
	initFixed(containerId: string, width: number, height: number) {
		this.loadScript();
		const container = document.getElementById(containerId);
