...
(function () {
  ...
  howTo.addEventListener('toggle', () => {
    if (!howTo.open || reported) return;
    reported = true;
    // trackEvent no-ops until consent is granted, so no gating needed here.
    if (window.trackEvent) window.trackEvent('howto_open', { slug, level });
  });
  ...
  document.addEventListener('click', e => {
    if (howTo.open && !howTo.contains(e.target as Node)) close();
  });
