...
interface ChartPadding { top: number; right: number; bottom: number; left: number; }
...
// Per-series label/colour override, keyed by 1-based column index.
// Only DualSeriesChart reads it; declared here so ChartLayer can carry it.
interface SeriesMeta { label?: string; color?: string; }
type SeriesMetaMap = Record<number, SeriesMeta>;

// One dataset drawn as its own layer, with per-layer draw mode.
...
// Chart uses the suffixes, DualSeriesChart uses the metas.
interface InputOutputOpts {
	inputSuffix?: string;
	outputSuffix?: string;
	inputMeta?: SeriesMetaMap;
	outputMeta?: SeriesMetaMap;
}
...
class Chart {
	...
	// Assigned in _buildDom / _bindEvents, not the constructor body.
	root!: HTMLDivElement;
	...
	protected _hover: ChartHover | null;
	protected _drawPending: boolean;
	protected _scales: ChartScales | null = null;
	...
	renderLayers(layers: ChartLayer[]) {
	...
	protected _buildDom() {
		...
		// getContext returns null when 2D is unavailable. Fail loudly here rather
		// than letting every later ctx call throw somewhere less obvious.
		const ctx = this.canvas.getContext('2d');
		if (!ctx) throw new Error('Chart: 2D canvas context unavailable');
		this.ctx = ctx;
	...
	protected _resizeCanvasForDPR(): { w: number; h: number } {
	...
	protected _computeScales(w: number, h: number): ChartScales {
	...
	static _niceTicks(min: number, max: number, count = 5): number[] {
	...
	protected _draw() {
		...
		ctx.restore();
		this._scales = { xMin, xMax, yMin, yMax, sx, sy, plotW, plotH };
	}

	protected _formatNum(v: number): string {
	...
	protected _onMouseMove(e: MouseEvent) {
		...
		let closest: ChartHover | null = null, closestDist = Infinity;
		for (const series of this._series) {
			for (const p of series.points) {
	...
	protected _showContextMenu(e: MouseEvent) {
