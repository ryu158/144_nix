 ...
 * keyed by 1-based column index. Pure addition — chart.ts is untouched.
 */
interface DualChartOptions extends ChartOptions {
  seriesMeta?: SeriesMetaMap;
}

// Label/colour options for the buildInputOutputMeta* helpers.
interface MetaOpts {
  labelA?: string;
  labelB?: string;
  colorsA?: string[];
...
class DualSeriesChart extends Chart {
  ...
  constructor(container: HTMLElement, options: DualChartOptions = {}) {
    super(container, options);
    this.seriesMeta = options.seriesMeta || {}; // { [colIndex]: {label, color} }
  }

  setSeriesMeta(meta: SeriesMetaMap | null | undefined) {
	...
	protected _parseDataWithMode(data2d: Grid2D, mode: Partial<ChartLayer> = {}): ChartSeries[] {
	...
	renderInputOutput(inputData2d: Grid2D, outputData2d: Grid2D, opts: InputOutputOpts = {}) {
		this.renderLayers([
  ...
  protected _parseData(data2d: Grid2D): ChartSeries[] {
	...
	// Convenience wrapper: takes two grid instances directly (instead of raw
	// data2d), builds seriesMeta automatically, and renders both layers.
	...
	plotFromGridsIndexed(gridA: GridSource, gridB: GridSource, opts: IndexedMetaOpts = {}) {
	...
	// Parallel to plotFromGridsIndexed, but using header-row-derived legend
	// labels instead of auto-generated column letters.
	plotFromGridsWithHeaders(gridA: GridSource, gridB: GridSource, opts: MetaOpts = {}) {
	...
	/**
	 * Legend grouping override. Groups legend entries by "_input"/"_output"
	 ...
	 */
	protected _renderLegend() {
		if (!this.showLegend) { this.legendEl.innerHTML = ''; return; }
		this.legendEl.innerHTML = '';

		const isInput = (s: ChartSeries) => /_input$/.test(s.label);
		const isOutput = (s: ChartSeries) => /_output$/.test(s.label);
		...
		const appendGroup = (group: ChartSeries[]) => {
			group.forEach(s => {
...
*/
function mergeGridsForChart(dataA: Grid2D, dataB: Grid2D, opts: MetaOpts = {}): { data: Grid2D; seriesMeta: SeriesMetaMap } {
  ...
  for (const row of dataA) {
    if (!row) continue;
    const merged = new Array<string>(totalCols).fill('');
    merged[0] = row[0] ?? '';
  ...
  const seriesMeta: SeriesMetaMap = {};
 ...
 */
function buildInputOutputMeta(dataA: Grid2D, dataB: Grid2D, opts: MetaOpts = {}): { inputMeta: SeriesMetaMap; outputMeta: SeriesMetaMap } {
	const labelA = opts.labelA || 'Input';
	const labelB = opts.labelB || 'Output';
	const colorsA = opts.colorsA || ['#19D3F3', '#EF553B', '#B6E880'];
	const colorsB = opts.colorsB || ['#636EFA', '#7F0000', '#00CC96'];
	...
	const outputMeta: SeriesMetaMap = {};
	for (let c = 0; c < yColsB; c++) {
		outputMeta[c + 1] = {
			label: yColsB > 1 ? `${labelB} ${Chart.colLabel(c + 1)}` : labelB,
			color: colorsB[c % colorsB.length],
 ...
 */
function _extractHeaderLabels(data2d: Grid2D): string[] {
	const headerRow = (data2d && data2d[0]) || [];
	return headerRow.map(v => (v ?? '').toString().trim());
}

function buildInputOutputMetaFromHeaders(dataA: Grid2D, dataB: Grid2D, opts: MetaOpts = {}): { inputMeta: SeriesMetaMap; outputMeta: SeriesMetaMap } {
	const labelA = opts.labelA || 'Input';
	const labelB = opts.labelB || 'Output';
 ...
 */
function buildInputOutputMetaIndexed(dataA: Grid2D, dataB: Grid2D, opts: IndexedMetaOpts = {}): { inputMeta: SeriesMetaMap; outputMeta: SeriesMetaMap } {
