...
class DualSeriesChart extends Chart {
    ...
    _parseDataWithMode(data2d, mode = {}) {
        ...
        if (mode.seriesMeta)
            this.seriesMeta = mode.seriesMeta;
        const series = this._parseData(data2d);
        this.seriesMeta = prevMeta; // restore, no lingering side effects
        series.forEach(s => {
            if (mode.showLines !== undefined)
                s.showLines = mode.showLines;
            if (mode.showPoints !== undefined)
                s.showPoints = mode.showPoints;
            if (mode.labelSuffix)
                s.label += mode.labelSuffix;
        });
        return series;
    }
    ...
    _parseData(data2d) {
        ...
        for (const row of data2d) {
            if (!row)
                continue;
            const xRaw = row[0];
            if (xRaw === undefined || xRaw === '' || xRaw === null)
                continue;
            const x = Number(xRaw);
            if (Number.isNaN(x))
                continue;
            for (let s = 0; s < seriesCount; s++) {
                const yRaw = row[s + 1];
                if (yRaw === undefined || yRaw === '' || yRaw === null)
                    continue;
                const y = Number(yRaw);
                if (Number.isNaN(y))
                    continue;
    ...
    plotFromGridsIndexed(gridA, gridB, opts = {}) {
    ...
    plotFromGridsWithHeaders(gridA, gridB, opts = {}) {
...
//# sourceMappingURL=dual-chart.js.map