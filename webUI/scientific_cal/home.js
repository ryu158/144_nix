...
(function () {
    ...
    function card(spec) {
        ...
        (spec.levels || []).forEach(level => {
            ...
            if (!href)
                return; // level declared but no page URL - skip rather than link nowhere
            ...
            a.addEventListener('click', () => {
                if (window.trackEvent)
                    window.trackEvent('topic_open', { slug: spec.slug, level: level });
            });
            row.appendChild(a);
        });
        el.appendChild(row);
    ...
    function render() {
        fetch('/scientific_cal/topics/topics.json')
            ...
            if (!list)
                return;
            const ok = specs.filter((s) => Boolean(s));
            list.innerHTML = '';
            if (!ok.length) {
            ...
            if (status)
                status.textContent = 'Could not load topics.';
        });
    }
